...
static dispatch_queue_t attributed_label_parse_queue;
static dispatch_queue_t get_attributed_label_parse_queue() \
{
    if (attributed_label_parse_queue == NULL) {
        attributed_label_parse_queue = dispatch_queue_create("com.parse_queue", 0);
    }
    return attributed_label_parse_queue;
}

@interface CoreTextDisplayView ()
{
...
@property (nonatomic,strong)    AttributedLabelURL *touchedLink;
...
- (void)setFont:(UIFont *)font
{
    if (font && _font != font)
    {
        ...
        [self resetFont];
        for (AttributedLabelAttachment *attachment in _attachments)
        {
...
- (NSAttributedString *)attributedStringForDraw
{
    if (_attributedString)
    {
        ...
        for (AttributedLabelURL *url in _linkLocations)
        {
            ...
            [drawString setUnderlines:_underLineForLink range:url.range underlinesColor:drawLinkColor];
        }
        return drawString;
    }
    else
...
- (AttributedLabelURL *)urlForPoint: (CGPoint)point
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (CGRectContainsPoint(rect, point))
        {
            ...
            AttributedLabelURL *url = [self linkAtIndex:idx];
            if (url)
            {
                return url;
            }
...
{
    AttributedLabelURL *url = [self urlForPoint:point];
...
- (AttributedLabelURL *)linkAtIndex:(CFIndex)index
{
    for (AttributedLabelURL *url in _linkLocations)
    {
...
- (void)appendAttachment: (AttributedLabelAttachment *)attachment
{
    attachment.fontAscent                   = _fontAscent;
    attachment.fontDescent                  = _fontDescent;
             ...
             margin: (UIEdgeInsets)margin
{
    [self appendImage:image
              maxSize:maxSize
               margin:margin
            alignment:ImageAlignmentBottom];
}
...
{
    AttributedLabelAttachment *attachment = [AttributedLabelAttachment attachmentWith:image
                                                                                     margin:margin
                                                                                  alignment:alignment
                                                                                    maxSize:maxSize];
...
- (void)appendView: (UIView *)view
         ...
         alignment: (ImageAlignment)alignment
{
    AttributedLabelAttachment *attachment = [AttributedLabelAttachment attachmentWith:view
                                                                                     margin:margin
                                                                                  alignment:alignment
                                                                                    maxSize:CGSizeZero];
    [self appendAttachment:attachment];
...
{
    AttributedLabelURL *url = [AttributedLabelURL urlWithLinkData:linkData
                                                                  range:range
                                                                  color:color];
...
//#pragma mark -
//+ (void)setCustomDetectMethod:(CustomDetectLinkBlock)block
//{
//    [AttributedLabelURL setCustomDetectMethod:block];
//}
...
- (void)drawAttachments
{
    ...
    for (CFIndex i = 0; i < numberOfLines; i++)
    {
        ...
        for (CFIndex k = 0; k < runCount; k++)
        {
            ...
            AttributedLabelAttachment* attributedImage = (AttributedLabelAttachment *)CTRunDelegateGetRefCon(delegate);
...
- (void)recomputeLinksIfNeeded
{
    ...
    BOOL sync = length <= MinAsyncDetectLinkLength;
               ...
               sync:(BOOL)sync
{
    ...
    LinkBlock block = ^(NSArray *links)
    {
        ...
        if ([links count])
        {
            for (AttributedLabelURL *link in links)
            {
    ...
    {
        _ignoreRedraw = YES;
        NSArray *links = [AttributedLabelURL detectLinks:text];
        block(links);
        _ignoreRedraw = NO;
    }
    else
    {
        dispatch_sync(get_attributed_label_parse_queue(), ^{
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    AttributedLabelURL *touchedLink = [self urlForPoint:point];
